...
async def main():
    ...
    history_list = await generate_history_list(task, model=my_browser_model, base_url=my_url, headless=headless)
    
    playwright_code_content = generate_playwright_code(history_list, model=my_code_model, base_url=my_url).content
    print("Playwright Code:\n")
    print(playwright_code_content)

    pytest_playwright_code = generate_pytest_playwright_code(playwright_code_content, model=my_code_model, base_url=my_url)
